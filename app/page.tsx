...
const QUESTIONS: Question[] = [
  { title: "You arrive early in a new ecosystem. What do you do first?", options: [
  ...
  { title: "A new protocol launches tonight. Your instinct is to…", options: [
  ...
  { title: "What keeps you coming back?", options: [
...
export default function Home() {
  ...
  async function resolveBirth(walletAddress: string) {
    ...
    } catch { setBirth({ status: "unavailable" }); }
  }

  async function connect() {
    setError("");
    const ethereum = (window as Window & { ethereum?: EthereumProvider }).ethereum;
    if (!ethereum) { setError("No injected wallet found. Open the app in a compatible wallet or install one first."); return; }
    try {
  ...
  function answer(archetype: number) {
    const nextAnswers = [...answers, archetype]; setAnswers(nextAnswers);
    if (questionIndex === QUESTIONS.length - 1) setStep("reveal"); else setQuestionIndex((current) => current + 1);
  }

  function restartQuiz() { setAnswers([]); setQuestionIndex(0); setName(""); setMint({ status: "idle" }); setStep("quiz"); }
  function birthLabel() {
  ...
  async function mintCompanion() {
    ...
    if (trimmedName.length < 2 || trimmedName.length > 24) { setMint({ status: "error", message: "Name must be 2–24 characters." }); return; }
    if (birth.status !== "found" && birth.status !== "new") { setMint({ status: "error", message: "Your Arc birth must be resolved before minting." }); return; }
    const ethereum = (window as Window & { ethereum?: EthereumProvider }).ethereum;
    if (!ethereum) { setMint({ status: "error", message: "Wallet provider is unavailable." }); return; }
  ...
  return (
    <main className="shell">
      <nav className="nav"><strong>ARC COMPANION</strong>{address ? <span className="network">{shortAddress(address)} · Arc Testnet</span> : <span className="network">Arc Testnet</span>}</nav>

      {step === "landing" && <section className="hero">
        <div className="copy"><p className="eyebrow">YOUR ARC JOURNEY, BROUGHT TO LIFE</p><h1>Meet the companion shaped by your history.</h1><p className="lede">Your first Arc activity marks their beginning. Your Web3 personality shapes who they are. What you do next shapes who they become.</p><button onClick={connect}>Discover my companion</button>{error && <p className="errorText">{error}</p>}<p className="micro">Built on Arc Testnet · Daily state lives onchain</p></div>
        <CompanionVisual label="A glimpse of what can awaken on Arc." mode="dormant" familyIndex={0} featured />
      </section>}

      {step === "quiz" && <section className="onboarding">
        <div className="quizPanel"><div className="quizTopline"><span>WEB3 PERSONALITY</span><span>{questionIndex + 1} / {QUESTIONS.length}</span></div><div className="progress"><i style={{ width: `${((questionIndex + 1) / QUESTIONS.length) * 100}%` }} /></div><h2>{QUESTIONS[questionIndex].title}</h2><div className="optionGrid">{QUESTIONS[questionIndex].options.map((option) => <button className="optionButton" key={option.label} onClick={() => answer(option.archetype)}>{option.label}</button>)}</div><p className="micro">No answer gives you an advantage. It only shapes your companion&apos;s genesis identity.</p></div>
      ...
      {step === "reveal" && result && <section className="onboarding revealSection">
        <div className="quizPanel revealCopy"><p className="eyebrow">GENESIS SIGNAL FOUND</p><h2>Meet your {result.family}.</h2><p className="lede">Your answers point to a <strong>{result.archetype}</strong> nature. Your wallet deterministically selected the {result.family} family.</p>
          <div className="genesisFacts"><div><span>ARCHETYPE</span><strong>{result.archetype}</strong></div><div><span>FAMILY</span><strong>{result.family}</strong></div><div><span>BORN ON ARC</span><strong>{birthLabel()}</strong></div></div>
          {result.family === "Vexa" ? <div className="evolutionTease"><span>EVOLUTION POTENTIAL</span><strong>Veyra · Vexus</strong><p>Your future choices will decide which path awakens.</p></div> : result.family === "Noma" ? <div className="evolutionTease"><span>EVOLUTION POTENTIAL</span><strong>Nymora · Noryx</strong><p>Silent signal, memory core and flowing presence define this Genesis form.</p></div> : <div className="evolutionTease"><span>VISUAL IDENTITY</span><strong>{result.family} is still concealed</strong><p>This family&apos;s production artwork will be revealed in a later build.</p></div>}
      ...
      <section className="principles"><article><strong>01</strong><span>History</span><p>Your real Arc beginning becomes part of the companion.</p></article><article><strong>02</strong><span>Care</span><p>A short daily moment becomes one persistent onchain update.</p></article><article><strong>03</strong><span>Growth</span><p>Streaks, choices and Arc achievements change the visual identity.</p></article></section>
    </main>
  );
}
